...
class cekunitController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        foreach ($records as $row) {
            ...
            $dataToInsert[] = [
                ...
                'jto' => $this->parseDate($cleanRow['jto'] ?? null),
    ...
    // Fungsi untuk mengkonversi berbagai format tanggal ke Y-m-d (format MySQL)
    private function parseDate($value)
    {
        ...
        // Format dengan garis miring: d/m/Y (Indonesia) → Y-m-d
        ...
        // Format dengan titik: d.m.Y → Y-m-d
        if (preg_match('#^(\d{1,2})\.(\d{1,2})\.(\d{4})$#', $value, $m)) {
            try {
                return Carbon::createFromFormat('d.m.Y', $value)->format('Y-m-d');
            } catch (\Exception $e) {
        ...
        // Format Y-m-d atau format lain yang didukung Carbon
        try {
